# -*- coding: utf-8 -*-
"""Question_and_Answer_System.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/15UYuX1MDWovQ5CrJYfbFckj_B0BvilZp
...
with open("data/train-v2.0.json", mode="rt", encoding="utf-8") as file:
...
data = get_SQuAD_data(qa_data, max_data_count, max_context_seq_length,
...
for sample in data:
    ...
    paragraph_word_list = [w.lower() for w in nltk.word_tokenize(
        paragraph) if in_white_list(w)]
    question_word_list = [w.lower() for w in nltk.word_tokenize(
        question) if in_white_list(w)]
    answer_word_list = [w.lower()
                        for w in nltk.word_tokenize(answer) if in_white_list(w)]
    ...
    input_paragraph_max_seq_length = max(
        input_paragraph_max_seq_length, len(paragraph_word_list))
    input_question_max_seq_length = max(
        input_question_max_seq_length, len(question_word_list))
...
input_paragraph_idx2word = dict(
    [(idx, word) for word, idx in input_paragraph_word2idx.items()])
input_question_idx2word = dict(
    [(idx, word) for word, idx in input_question_word2idx.items()])
...
for input_data, output_data in zip(input_data_samples, output_data_samples):
    ...
    input_encoded_data_samples.append(
        [input_paragraph_encoded_data, input_question_encoded_data])
...
x_train, x_test, y_train, y_test = train_test_split(
    input_encoded_data_samples, target_encoded_data_samples, test_size=test_size, random_state=random_state)
    ...
    while True:
        for batchIdx in range(0, num_batches):
            ...
            decoder_target_data_batch = np.zeros(
                shape=(batch_size, target_max_seq_length, num_target_tokens))
...
train_gen = generate_batch(input_paragraph_max_seq_length, input_question_max_seq_length,
                           target_max_seq_length, num_target_tokens, x_train, y_train, batch_size)
test_gen = generate_batch(input_paragraph_max_seq_length, input_question_max_seq_length,
                          target_max_seq_length, num_target_tokens, x_test, y_test, batch_size)
...
def create_model(num_encoder_paragraph_tokens, max_encoder_paragraph_seq_length,
    ...
    encoded_question = RepeatVector(
        max_encoder_paragraph_seq_length)(encoded_question)
    ...
    model = Model([context_inputs, question_inputs,
    ...
    decoder_state_inputs = [
        Input(shape=(hidden_units,)), Input(shape=(hidden_units,))]
...
def reply(paragraph, question):
    ...
    input_paragraph_seq = pad_sequences(
        input_paragraph_seq, input_paragraph_max_seq_length)
    input_question_seq = pad_sequences(
        input_question_seq, input_question_max_seq_length)
...
# #lstm_predicted = encoder_model.predict([input_paragraph_seq, input_question_seq])
# lstm_predicted_tags = []
...
"""take paragraph and question And try to implement the f1 score"""
...
#   maxx=0
#   if(len(actual_ans) > len(pred_ans)):
#     temp = actual_ans
#   else:
#     temp = pred_ans
...
for i in range(10):
    ...
    #compute(predicted_answer, actual_answer)
...
def compute(pred, act):
    pred_tokens = normalize_text(pred)
    truth_tokens = normalize_text(act)
...
predicted_answer = 'destinys child'


pred_tokens = normalize_text(predicted_answer)
print(pred_tokens)
set(pred_tokens)

#compute(actual_answer, predicted_answer)

actual_answer = 'romantic'

truth_tokens = normalize_text(actual_answer)
print(truth_tokens)
set(truth_tokens)
...
#t = (int(normalize_text(predicted_answer) == normalize_text(actual_answer)))
# print(t)


def question_answer(question, reviews_text):

    return reply(reviews_text, question)
